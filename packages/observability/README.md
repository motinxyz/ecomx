...
2. **Standardized Logging**: Exporting `createLogger()`, a perfectly configured synchronous Pino instance guaranteed to preserve OTel Trace Context across asynchronous boundaries.
3. **Framework Middleware**: Exporting plug-and-play middleware (e.g. `elysiaObservabilityPlugin`) to instantly trace HTTP routers.

## 🛑 What is Forbidden Here
*   **Domain Specific Metrics**: This package provides the **Engine**, not the **Data**. You may *not* define `recordLogin()`, `recordCheckout()`, or `recordDatabaseError()` here. If you need a specialized counter, you must define it locally inside the microservice (`@ecomx/auth-service/src/metrics.ts`).
